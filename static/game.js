...
socket.on("viewclient", function(room) {
    ...
    if (room.view === "lobby") {
		...
		<div class="col s12 center-align">
			<a class="waves-effect waves-light btn-small ${mainColor} lighten-1 ${canStart}" onClick="ioSend('startroom');">${
                elem.startbutton[lang]
            }</a>
		</div>
	`;
        }
    ...
    if (room.view === "raise" || room.view === "point" || room.view === "count" || room.view === "face" || room.view === "word") {
        templateRow(1, 2, "center-align");
    ...
    if (room.view === "raiseq" || room.view === "pointq" || room.view === "countq" || room.view === "faceq" || room.view === "wordq") {
        let question = "";
        let currentGame = "";
    ...
    if (room.view === "vote") {
        ...
        room.playerscore.forEach((player, i) => {
            if (player.nickname !== username) {
                playerButtons += `
				<div class="col s6 center-align">
					<a id="${player.nickname}" class="waves-effect waves-light btn ${ifc.config.playercolors[i]} lighten-1 mtop1" onClick="ioSendVote('${
                    player.nickname
                }');"><h5>${player.nickname}</h5></a>
				</div>
				`;
            }
        });
